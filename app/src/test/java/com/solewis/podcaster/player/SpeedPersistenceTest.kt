 ...
 * changes, [PlaybackSettings] hands it back when the player is rebuilt.
 *
...
class SpeedPersistenceTest {
    ...
    fun speed_is_normal_until_something_chooses_otherwise() {
        assertThat(PlaybackSettings(context).speed).isEqualTo(1f)
    }

    @Test
    fun a_chosen_speed_is_read_back_by_a_separate_reader() {
        PlaybackSettings(context).speed = 1.5f
        ...
        assertThat(PlaybackSettings(context).speed).isEqualTo(1.5f)
    }
